...
function Slackbot(configuration) {
    ...
    slack_botkit.getAuthorizeURL = function(params) {
        ...
        url = url + '?client_id=' + slack_botkit.config.clientId + '&scope=' +
            scopes.join(',') + '&state=' + (params && params.state ? params.state : 'botkit');
    ...
    slack_botkit.createOauthEndpoints = function(webserver, callback) {
        ...
        webserver.get('/login', function(req, res) {
            ...
            res.redirect(slack_botkit.getAuthorizeURL(params));
        });
                        ...
                        if (err) {
                            ...
                            slack_botkit.findTeamById(identity.team_id, function(err, team) {
                                ...
                                if (auth.incoming_webhook) {
                                    ...
                                    slack_botkit.trigger('create_incoming_webhook', [bot, team.incoming_webhook, state]);
                                }
                                ...
                                if (auth.bot) {
                                    ...
                                    slack_botkit.trigger('create_bot', [bot, team.bot, state]);
                                }
                                ...
                                slack_botkit.saveTeam(team, function(err, id) {
                                    if (err) {
                                        ...
                                        if (isnew) {
                                            slack_botkit.trigger('create_team', [bot, team, state]);
                                        } else {
                                            slack_botkit.trigger('update_team', [bot, team, state]);
                                        }
                                        ...
                                        slack_botkit.storage.users.get(identity.user_id, function(err, user) {
                                            ...
                                            slack_botkit.storage.users.save(user, function(err, id) {
                                                ...
                                                if (err) {
                                                    ...
                                                    if (isnew) {
                                                        slack_botkit.trigger('create_user', [bot, user, state]);
                                                    } else {
                                                        slack_botkit.trigger('update_user', [bot, user, state]);
                                                    }
                                                    if (callback) {
                                                        callback(null, req, res, { bot:bot, team: team, user: user, state: state });
                                                    } else {
                                                        res.redirect('/');
                                                    }
                                                }
